...
class _ShopReturnPageState extends State<ShopReturnPage> {
  ...
  void _addItem(OrderItem product) {
    final existing = _items.where((i) => (i.productId != null && i.productId == product.productId) || i.productNameSnapshot == product.productName);
    if (existing.isNotEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('${product.productName} is already in the list. Adjust the quantity below.')),
      );
      return;
    }
    setState(() {
  ...
  Future<void> _submit() async {
    ...
    if (pin.length != 6) {
      setState(() { _error = 'Enter the 6-digit confirmation PIN.'; });
      return;
    }
    for (final item in _items) {
      if (item.reason == 'OTHER' && (item.reasonNote == null || item.reasonNote!.trim().isEmpty)) {
        setState(() { _error = 'Describe the reason for "${item.productNameSnapshot}".'; });
        return;
      }
    }
    setState(() { _submitting = true; _error = null; });
    try {
      await _service.submitShopReturn(orderId: widget.order.orderId, pin: pin, items: _items);
              ...
              child: Column(
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: Row(children: [
                      Icon(Icons.store_outlined, color: AppTheme.primaryBrown, size: 20),
                      const SizedBox(width: 8),
                      Expanded(child: Text('${widget.order.shopName} · ${widget.order.orderCode}',
                        style: theme.textTheme.bodyMedium?.copyWith(fontWeight: FontWeight.w600, color: AppTheme.textDark),
                      )),
                    ]),
                  ),
                  ...
                  // Product search
                  Text('Select Products to Return', style: theme.textTheme.titleSmall?.copyWith(fontWeight: FontWeight.w700, color: AppTheme.textDark)),
                  ...
                  if (_filteredInventory.isEmpty)
                    Padding(
                      ...
                      child: Text('No matching products found on the lorry.', style: theme.textTheme.bodySmall?.copyWith(color: AppTheme.textSoft)),
                    )
                  else
                    ...(_filteredInventory.map((product) => Padding(
                      padding: const EdgeInsets.only(bottom: 6),
                      child: _ProductRow(product: product, onAdd: () => _addItem(product)),
                    ))),
                  ...
                  if (_items.isNotEmpty) ...[
                    ...
                    const SizedBox(height: 8),
                    ..._items.asMap().entries.map((entry) => _ReturnItemCard(
                      index: entry.key,
                      item: entry.value,
                    ...
                    Container(
                      ...
                      child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                        Text('Estimated Return Value', style: theme.textTheme.bodyMedium?.copyWith(fontWeight: FontWeight.w600, color: AppTheme.textDark)),
                        Text('LKR ${_totalValue.toStringAsFixed(2)}',
                          style: theme.textTheme.titleSmall?.copyWith(fontWeight: FontWeight.w800, color: AppTheme.primaryBrownDark),
                        ),
                      ]),
                    ),
                  ],
                  ...
                  if (!_pinSent) ...[
                    Text('Confirm with Shop Owner', style: theme.textTheme.titleSmall?.copyWith(fontWeight: FontWeight.w700, color: AppTheme.textDark)),
                    const SizedBox(height: 6),
                    Text('A confirmation PIN will be sent to the shop owner\'s app. They will share it with you to authorise the product pickup.',
                    ...
                    Container(
                      ...
                      child: Row(children: [
                        ...
                        const SizedBox(width: 8),
                        Expanded(child: Text('PIN sent to shop owner. Ask them to share the 6-digit code.',
                          style: theme.textTheme.bodySmall?.copyWith(color: const Color(0xFF1E5C3A)),
                        )),
                      ]),
                    ),
                    const SizedBox(height: 16),
                    Text('Enter Confirmation PIN', style: theme.textTheme.titleSmall?.copyWith(fontWeight: FontWeight.w700, color: AppTheme.textDark)),
                    ...
                    Row(
                      ...
                      children: List.generate(6, (index) => SizedBox(
                        ...
                        child: TextField(
                          ...
                          maxLength: 1,
                          style: Theme.of(context).textTheme.headlineMedium?.copyWith(fontWeight: FontWeight.w800, color: AppTheme.textDark),
                    ...
                    const SizedBox(height: 8),
                    Center(child: TextButton(onPressed: _requestPin, child: const Text('Resend PIN'))),
                    const SizedBox(height: 20),
                    FilledButton.icon(
                      ...
                      label: const Text('Record Return', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w700)),
                      style: FilledButton.styleFrom(
                        backgroundColor: AppTheme.primaryBrownDark,
    ...
    return Container(
      ...
      child: Row(children: [
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(product.productName, style: Theme.of(context).textTheme.bodyMedium?.copyWith(fontWeight: FontWeight.w600, color: AppTheme.textDark)),
          Text('${product.quantity} case(s) on lorry · LKR ${product.unitPrice.toStringAsFixed(2)}/case',
            style: Theme.of(context).textTheme.bodySmall?.copyWith(color: AppTheme.textSoft),
          ),
        ])),
        IconButton(
          onPressed: onAdd,
...
class _ReturnItemCard extends StatelessWidget {
  const _ReturnItemCard({required this.index, required this.item, required this.onRemove, required this.onChanged});
  final int index;
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          Expanded(child: Text(item.productNameSnapshot, style: theme.textTheme.titleSmall?.copyWith(fontWeight: FontWeight.w700, color: AppTheme.textDark))),
          IconButton(icon: const Icon(Icons.close, size: 18, color: Color(0xFF9B4B46)), onPressed: onRemove, visualDensity: VisualDensity.compact),
        ]),
        const SizedBox(height: 8),
        Row(children: [
          Expanded(child: DropdownButtonFormField<String>(
            ...
            onChanged: (v) { if (v != null) { item.unitType = v; onChanged(); } },
          )),
          const SizedBox(width: 8),
          Expanded(child: TextFormField(
            initialValue: item.quantity.toString(),
            decoration: const InputDecoration(labelText: 'Quantity', contentPadding: EdgeInsets.symmetric(horizontal: 12, vertical: 10)),
            keyboardType: TextInputType.number,
            inputFormatters: [FilteringTextInputFormatter.digitsOnly],
        ...
        if (item.reason == 'OTHER') ...[
          ...
          TextFormField(
            ...
            decoration: const InputDecoration(labelText: 'Describe reason', contentPadding: EdgeInsets.symmetric(horizontal: 12, vertical: 10)),
            onChanged: (v) { item.reasonNote = v; onChanged(); },
          ),
        ],
        if (item.unitPrice != null && item.unitPrice! > 0) ...[
          const SizedBox(height: 8),
          Text('Value: LKR ${item.totalValue.toStringAsFixed(2)}',
            style: theme.textTheme.bodySmall?.copyWith(color: AppTheme.primaryBrownDark, fontWeight: FontWeight.w600),
          ),
        ],
      ]),
    );
    ...
    return Center(
      child: Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          const SizedBox(height: 16),
          Text('Return Recorded', textAlign: TextAlign.center,
            style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.w800, color: AppTheme.textDark),
          ),
          const SizedBox(height: 8),
          Text('Products have been logged as returned. Your territory manager has been notified.',
            textAlign: TextAlign.center,
          ...
          FilledButton(
            onPressed: onDone,
            style: FilledButton.styleFrom(backgroundColor: AppTheme.primaryBrown, minimumSize: const Size(200, 50), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16))),
